...
module.exports = {
   ...
   */
  saveBase64Image: (stream, filename) =>
  ...
  /**
   * Checks an email string against the pattern `{string}@{string}.{string}`.
   *
   * @param {String} email The email to validate.
   * @returns {boolean} `true` if email matches pattern, `false` if not.
   */
  validateEmail: (email) => {
    const regex = /^\S+@\S+\.\S+/;
